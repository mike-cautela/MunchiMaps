    ...
    if (err) {
        ...
        db.run('CREATE TABLE IF NOT EXISTS building \
            ...
            if (err) {
                ...
                console.log('The buildings table was created successfully');
            }
        });
        ...
        db.run('CREATE TABLE IF NOT EXISTS review ( \
            ...
            if (err) {
                ...
                console.log('The reviews table was created successfully');
            }
        });
    }
});
...
async function insertBuilding(name, x_coord, y_coord, time_opens, time_closes, num_snack_machines, num_drink_machines, num_ratings, average_ratings, needs_service) {
    return new Promise((resolve, reject) => {
        db.run('INSERT INTO building (name, x_coord, y_coord, time_opens, time_closes, num_snack_machines, num_drink_machines, num_ratings, average_ratings, needs_service) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
        [name, x_coord, y_coord, time_opens, time_closes, num_snack_machines, num_drink_machines, num_ratings, average_ratings, needs_service], function(err) {
            if (err) {
                reject(err);
            } else {
...
async function insertReview(building_id, product_rating, functionality_rating, needs_service) {
    return new Promise((resolve, reject) => {
        db.run('INSERT INTO review (building_id, product_rating, functionality_rating, needs_service) VALUES (?, ?, ?, ?)', 
        [building_id, product_rating, functionality_rating, needs_service], function(err) {
            if (err) {
                reject(err);
            } else {
